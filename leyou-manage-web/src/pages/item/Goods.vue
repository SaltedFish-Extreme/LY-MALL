<template>
  <v-card>
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          <v-btn @click="editGoods(props.item)" icon>
    ...
    <!--弹出的对话框-->
    <v-dialog max-width="800" persistent scrollable v-model="show">
      <v-card>
        <!--对话框的标题-->
        ...
        <v-card-text class="px-3" style="height: 600px">
          <goods-form :is-edit="isEdit" :oldGoods="oldGoods" :step="step" @close="closeWindow" ref="goodsForm"/>
        </v-card-text>
        <!--底部按钮，用来操作步骤线-->
        <v-card-actions class="elevation-10">
          <v-flex class="xs3 mx-auto">
            <v-btn :disabled="step === 1" @click="previous" color="primary">上一步</v-btn>
            <v-btn :disabled="step === 4" @click="next" color="primary">下一步</v-btn>
          </v-flex>
        </v-card-actions>
      </v-card>
    </v-dialog>
