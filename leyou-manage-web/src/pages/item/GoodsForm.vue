  ...
  <v-stepper v-model="step">
    ...
    <v-stepper-items>
      ...
      <v-stepper-content step="1">
        <v-flex class="xs10 mx-auto">
          <v-form ref="basic" v-model="valid">
            ...
            <v-text-field :counter="200" :rules="[v => !!v || '商品标题不能为空']" hide-details label="商品标题" required
                          v-model="goods.title"/>
            <v-text-field :counter="200" hide-details label="商品卖点" v-model="goods.subTitle"/>
            <v-text-field :counter="1000" :rows="3" hide-details label="包装清单" multi-line
                          v-model="goods.spuDetail.packingList"/>
            <v-text-field :counter="1000" :rows="3" hide-details label="售后服务" multi-line
      ...
      <v-stepper-content step="2">
        <v-editor upload-url="/upload/image" v-model="goods.spuDetail.description"/>
      </v-stepper-content>
      <!--3、规格参数-->
        ...
        <v-flex class="xs10 mx-auto px-3">
          ...
          <v-card class="my-2">
            <v-container fluid grid-list-md>
              <v-layout class="px-5" justify-space-between row wrap>
                <v-flex :key="param.name" sm5 v-for="param in specs" xs12>
                  <v-text-field :label="param.name" :suffix="param.unit || ''" v-model="param.v"
                  />
                </v-flex>
              </v-layout>
            </v-container>
      ...
      <v-stepper-content step="4">
        <v-flex class="mx-auto">
          <!--遍历特有规格参数-->
          <v-card :key="spec.name" flat v-for="spec in specialSpecs">
            <!--特有参数的标题-->
            <div class="subheading">{{spec.name}}:</div>
            <!--特有参数的待选项，需要判断是否有options，如果没有，展示文本框，让用户自己输入-->
            <v-card-text class="px-5">
              <div :key="i" class="layout row px-5" v-for="i in spec.options.length+1">
                <v-text-field :placeholder="'新的' + spec.name + ':'" auto-grow class="flex xs10"
                              hide-details single-line v-bind:value="i" v-model="spec.options[i-1]"/>

                <v-btn @click="spec.options.splice(i-1,1)" icon v-if="i <= spec.options.length">
            ...
            <v-data-table :headers="headers" :items="skus" class="elevation-0" hide-actions item-key="indexes">
              <template slot="items" slot-scope="props">
                <tr @click="props.expanded = !props.expanded">
                  <!--价格和库存展示为文本框-->
                  <td :key="k" class="text-xs-center" v-for="(v,k) in props.item"
                      v-if="['price', 'stock'].includes(k)">
                    <v-text-field @click.stop="" single-line v-model="props.item[k]"/>
                  </td>
                  <!--enable展示为checkbox-->
              ...
              <template slot="expand" slot-scope="props">
                <v-card class="elevation-2 flex xs11 mx-auto my-2">
                  <!--图片上传组件-->
                  <v-upload url="/upload/image" v-model="props.item.images"/>
                </v-card>
              </template>
            </v-data-table>
          </v-card>
        ...
        <v-flex offset-xs9 xs3>
          <v-btn @click="submit" color="info">保存商品信息</v-btn>
        </v-flex>
      </v-stepper-content>
    </v-stepper-items>
  </v-stepper>
