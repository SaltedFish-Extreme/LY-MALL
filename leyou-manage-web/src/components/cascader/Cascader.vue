<template>
  <cas :data="options" :loadData="loadOption" @on-change="handleChange" transfer>
    <v-select :label="label" :required="required" :rules="defaultRules" chips clearable tags
              v-model="selected">
      <template slot="selection" slot-scope="data">
        <v-chip @click.stop="" @input="remove(data.item)" close color="green" outline small v-if="multiple">
          {{ data.item.label}}&nbsp;
        </v-chip>
        <v-chip @click.stop="" small v-else>{{ data.item}}</v-chip>
      </template>
    </v-select>
  </cas>
</template>
