<template>
  <div>
    <v-list-tile
      :class="{'selected':isSelected}" @click="toggle" class="level1 py-0 my-0">
      ...
      <v-list-tile-content>
        ...
        <input :ref="model.id" @blur="afterEdit" @click.stop="" @keydown.enter="afterEdit"
               v-model="model.name" v-show="beginEdit"/>
      </v-list-tile-content>
      <v-list-tile-action v-if="isEdit">
        <v-btn :color="c1" @click.stop="addChild" @mouseout="c1=''" @mouseover="c1='primary'" icon>
          <i class="el-icon-plus"/>
        </v-btn>
      </v-list-tile-action>
      <v-list-tile-action v-if="isEdit">
        <v-btn :color="c2" @click.stop="editChild" @mouseout="c2=''" @mouseover="c2='success'" icon>
          <i class="el-icon-edit"/>
        </v-btn>
      </v-list-tile-action>
      <v-list-tile-action v-if="isEdit">
        <v-btn :color="c3" @click.stop="deleteChild" @mouseout="c3=''" @mouseover="c3='error'" icon>
          <i class="el-icon-delete"/>
        </v-btn>
      </v-list-tile-action>
    </v-list-tile>

    <v-list class="ml-4 pt-0 pb-0" dense transition="scale-transition" v-if="isFolder" v-show="open">
      <tree-item
        :isEdit="isEdit"
        :key="index"
        :model="model"
